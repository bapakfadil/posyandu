                        ...
                        <tbody>
                            @foreach ($periksaKehamilans as $periksaKehamilan)
                                <tr>
                                    ...
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ \Carbon\Carbon::parse($periksaKehamilan->tanggal_periksa)->translatedFormat('d F Y') }}</td>
                                    ...
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <a href="{{ route('periksa_kehamilan.show', $periksaKehamilan->id) }}" class="inline-flex items-center px-2 py-1 bg-transparent hover:bg-blue-500 text-blue-700 rounded-md uppercase tracking-widest focus:outline-none border border-blue-500 hover:border-transparent disabled:opacity-25 transition">
                                        ...
                                        @if (Auth::user()->role == 'admin')
                                            <a href="{{ route('periksa_kehamilan.edit', $periksaKehamilan->id) }}" class="inline-flex items-center px-2 py-1 bg-transparent hover:bg-yellow-500 text-yellow-700 rounded-md uppercase tracking-widest focus:outline-none border border-yellow-500 hover:border-transparent disabled:opacity-25 transition">
                                                <img class="w-5 h-5" width="24" height="24" src="https://img.icons8.com/pastel-glyph/64/create-new--v2.png" alt="create-new--v2"/>
                                            </a>
                                            <form action="{{ route('periksa_kehamilan.destroy', $periksaKehamilan->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @endif
                                        <a href="{{ route('periksa_kehamilan.cetak', $periksaKehamilan->id) }}" class="inline-flex items-center px-2 py-1 bg-transparent hover:bg-green-500 text-green-700 rounded-md uppercase tracking-widest focus:outline-none border border-green-500 hover:border-transparent disabled:opacity-25 transition">
